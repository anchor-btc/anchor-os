...
| Carrier | ID | Description | Use Case |
|---------|-----|-------------|----------|
| OP_RETURN | `0` | Standard Bitcoin output | Fast, simple proofs up to 80 bytes |
| Inscription | `1` | Ordinals-style inscription | Permanent, larger metadata |
| Stamps | `2` | Bare multisig (unprunable) | Maximum permanence |
| Witness Data | `4` | Tapscript witness | 75% fee discount, large payloads |

::: tip Carrier Selection
- **OP_RETURN**: Best for minimal proofs (hash only, no metadata)
- **Witness Data**: Recommended for proofs with metadata (75% cheaper)
- **Inscription**: Use when you want the proof to be an Ordinal
- **Stamps**: Use when unprunability is critical
:::
...
### Create a Proof with Carrier Selection

```typescript
async function createProof(
  wallet: AnchorWallet,
  file: File,
  carrier: CarrierType = CarrierType.OpReturn
): Promise<string> {
  // Hash the file
  ...
  // Choose carrier based on payload size
  const effectiveCarrier = payload.length > 70 && carrier === CarrierType.OpReturn
    ? CarrierType.WitnessData  // Fallback to witness for large payloads
    : carrier
...
// Usage with different carriers
const proofOpReturn = await createProof(wallet, doc, CarrierType.OpReturn)
const proofWitness = await createProof(wallet, doc, CarrierType.WitnessData)
const proofInscription = await createProof(wallet, doc, CarrierType.Inscription)
```
...
async function createBatchProof(
  wallet: AnchorWallet,
  files: File[],
  carrier: CarrierType = CarrierType.WitnessData
): Promise<string> {
  ...
  // Batch proofs usually need larger carriers
  const effectiveCarrier = payload.length > 70
