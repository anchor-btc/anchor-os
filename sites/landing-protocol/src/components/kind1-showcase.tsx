      ...
      <div className="max-w-5xl mx-auto">
        ...
        >
          ...
          <p className="text-muted-foreground max-w-2xl mx-auto">
            The most common message type. Perfect for social posts, comments, and annotations. UTF-8
            encoded, human-readable, and permanent.
          </p>
        </motion.div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          >
            ...
            <div className="bg-muted/30 border border-muted rounded-xl p-4">
              <div className="text-xs text-muted-foreground mb-2 font-medium">HEX OUTPUT</div>
            ...
            {/* Breakdown */}
            <div className="bg-muted/30 border border-muted rounded-xl p-4">
              <div className="text-xs text-muted-foreground mb-3 font-medium">BREAKDOWN</div>
              <div className="space-y-2">
                {DECODED_PARTS.map((part) => (
                  <div key={part.label} className="flex items-center justify-between">
                    ...
                    <span className={`font-mono text-xs ${part.color}`}>{part.value}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-accent/10 border border-accent/20 rounded-xl p-4">
              <div className="text-xs text-accent mb-2 font-medium">DECODED</div>
...
function highlightTsSyntax(line: string): string {
  return line
    .replace(/(import|from|const|await|async)/g, '<span class="text-purple-400">$1</span>')
    .replace(/('[@\w\/\-.]+')/g, '<span class="text-green-400">$1</span>')
    .replace(/(AnchorKind|createMessage|broadcast)/g, '<span class="text-blue-400">$1</span>')
    .replace(/(\/\/.*)/g, '<span class="text-muted-foreground">$1</span>')
    .replace(/(kind|body|txid|result|message|wallet)/g, '<span class="text-cyan-400">$1</span>')
...
function highlightRustSyntax(line: string): string {
  ...
  processedLine = processedLine
    ...
    .replace(/\b(Message|Kind|Carrier|Text)\b/g, '<span class="text-blue-400">$1</span>')
    .replace(/(\/\/.*)/g, '<span class="text-muted-foreground">$1</span>')
    .replace(
