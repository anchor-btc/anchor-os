...
The **DNS** kind enables decentralized domain name registration and management on Bitcoin. It powers Anchor DNS with `.bit` domain names.
...
| 2+ | name | utf8 | Domain name (e.g., "example.bit") |
...
// Register example.bit pointing to 192.168.1.1
...
function isValidDomainName(name: string): boolean {
  // Must end with .bit
  if (!name.endsWith('.bit')) return false
