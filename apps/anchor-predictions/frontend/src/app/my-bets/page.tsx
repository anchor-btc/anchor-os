...
import { fetchMyPositions, fetchAllPositions, claimWinnings } from '@/lib/api';
import { cn, formatSats, formatPercent, shortenHash } from '@/lib/utils';
import { useWallet } from '@/contexts';

export default function MyBetsPage() {
  const { address, connected, pubkey: walletPubkey, signMessage } = useWallet();
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-2 bg-white/5 rounded-lg p-1">
          <button
            ...
            className={cn(
              ...
              viewMode === 'all' ? 'bg-amber-600 text-white' : 'text-gray-400 hover:text-white'
            )}
          >
            All Bets
          </button>
          <button
            ...
            className={cn(
              'px-4 py-2 rounded-md text-sm font-medium transition-colors',
              viewMode === 'mine' ? 'bg-amber-600 text-white' : 'text-gray-400 hover:text-white'
            )}
          >
            My Bets
          </button>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-400 text-sm">
                Connect your wallet to view your bets automatically
              </p>
            </div>
          </div>
        </div>
      ...
      {isLoading && searchPubkey ? (
            ...
            >
              ...
              <div className="flex items-center justify-between text-sm">
                <div className="text-gray-400">
                  <span>
                    Shares: <span className="text-white">{position.shares.toLocaleString()}</span>
                  </span>
                  <span className="mx-2">•</span>
                  <span>
                    Avg Price:{' '}
                    <span className="text-white">{formatPercent(position.avg_price)}</span>
                  </span>
                </div>
                {position.is_winner && (
        ...
        <div className="text-center py-12 rounded-xl border border-white/10 bg-white/5">
          ...
          <p className="text-sm text-gray-500">This public key has no positions in any markets</p>
        </div>
      ) : (
        <div className="text-center py-12 rounded-xl border border-white/10 bg-white/5">
          <User className="w-12 h-12 mx-auto text-gray-600 mb-4" />
          <p className="text-gray-400 mb-2">Enter your public key</p>
          <p className="text-sm text-gray-500">View all your bets across prediction markets</p>
        </div>
      )}
