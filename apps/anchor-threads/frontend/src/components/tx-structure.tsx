...
import { ChevronDown, ChevronUp, Loader2, Copy, Check, AlertCircle, X } from 'lucide-react';
  ...
  function parseHexIntoParts(hex: string): HexPart[] {
    ...
    if (isSegwit && pos < hex.length - 8) {
      ...
      if (remaining.length > 0) {
        const hasAnchor =
          remaining.toLowerCase().includes('414e43484f52') || remaining.includes(MAGIC_BYTES);
        parts.push({
          hex: remaining,
          type: hasAnchor ? 'anchor' : 'witness',
          label: 'Witness Data',
        });
        pos = hex.length - 8;
      }
      ...
      <div className="border rounded-lg overflow-hidden">
        <div className="flex items-center justify-between px-3 py-2 bg-muted/50 border-b text-sm">
          <span className="text-muted-foreground">
            Raw Hex ({txHex ? txHex.length / 2 : 0} bytes)
          </span>
          <button
            onClick={handleCopy}
                        ...
                        <div className="mt-2 pt-2 border-t border-border/50">
                          ...
                          <div className="space-y-1 max-h-32 overflow-y-auto">
                            {inp.txinwitness.map((w, wi) => {
                              const hasAnchor =
                                w.toLowerCase().includes('414e43484f52') || w.includes('a11c0001');
                              return (
                ...
                <div className="space-y-1">
                  {decoded.vout.map((out, i) => {
                    const isAnchor =
                      ...
                      out.scriptPubKey.hex.includes('a11c0001');
                    return (
