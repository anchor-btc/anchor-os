...
#[openapi(
    ...
    info(
        title = "BitDNS API",
        version = "1.0.0",
        description = "Decentralized DNS on Bitcoin using the Anchor Protocol"
    )
)]
struct ApiDoc;
...
async fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::registry()
        ...
        .with(tracing_subscriber::EnvFilter::from_default_env()
            .add_directive("bitdns_backend=info".parse()?)
            .add_directive("tower_http=debug".parse()?))
        .init();

    // Load configuration
    let config = Config::from_env();
    info!("Starting BitDNS Backend on port {}", config.port);
