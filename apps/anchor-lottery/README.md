...
Anchor Lottery enables provably fair lotteries where winnings are automatically paid out through Discreet Log Contracts (DLCs). Oracles from the Anchor Oracles network attest to the winning numbers, triggering trustless settlement.
...
```bash
# Start the database
docker compose up -d anchor-app-lottery-postgres

# Run the backend
cd apps/anchor-lottery/backend
cargo run

# Run the frontend
cd apps/anchor-lottery/frontend
npm run dev
```
