//! Data models for the BitDNS API
...
/// Validate a .bit domain name
pub fn is_valid_domain_name(name: &str) -> bool {
    // Must end with .bit
    if !name.ends_with(".bit") {
        return false;
    }

    // Get the name part (without .bit)
    let name_part = &name[..name.len() - 4];
...
mod tests {
    ...
    fn test_valid_domain_names() {
        ...
        assert!(is_valid_domain_name("my-site.bit"));
        assert!(is_valid_domain_name("sub.domain.bit"));
        assert!(is_valid_domain_name("a.bit"));
        assert!(is_valid_domain_name("test123.bit"));
    }

    #[test]
    fn test_invalid_domain_names() {
        assert!(!is_valid_domain_name("test.com"));
        assert!(!is_valid_domain_name(".bit"));
        assert!(!is_valid_domain_name("-test.bit"));
        assert!(!is_valid_domain_name("test-.bit"));
        assert!(!is_valid_domain_name("test..bit"));
    }
    ...
    fn test_dns_payload_roundtrip() {
        ...
        assert_eq!(decoded.name, "test.bit");
    ...
    fn test_mx_record() {
        ...
        assert_eq!(decoded.records[0].value, "mail.test.bit");
    }
}
