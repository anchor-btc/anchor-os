...
impl Database {
    ...
    pub async fn update_domain(
        ...
        // When a domain is updated, the new transaction becomes the proof of ownership
        sqlx::query(
            r#"
            UPDATE domains 
    ...
    /// 
    /// This is called immediately after a successful update transaction broadcast,
    /// BEFORE the transaction is confirmed. This prevents double-spend issues when
    /// multiple updates are requested in quick succession.
    /// 
    /// The indexer will later update the full domain data (records, block info)
    ...
    pub async fn get_domains_by_owner_txids(
        ...
        // Build a query with ANY for the list of txids
        let rows: Vec<(i32, String, Vec<u8>, i64, Option<i32>, chrono::DateTime<chrono::Utc>)> =
            sqlx::query_as(
                r#"
                SELECT d.id, d.name, d.txid, 
