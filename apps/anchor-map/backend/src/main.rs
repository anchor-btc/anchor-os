...
async fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::registry()
        .with(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "anchormap_backend=debug,tower_http=debug".into()),
        )
    ...
    info!("Starting AnchorMap Backend on {}:{}", config.host, config.port);
    ...
    info!("AnchorMap Backend listening on {}", addr);
