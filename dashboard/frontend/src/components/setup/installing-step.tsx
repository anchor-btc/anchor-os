...
export function InstallingStep({
  ...
  const parseLogType = useCallback((text: string): { type: LogLine['type']; cleanText: string } => {
    if (text.startsWith('[SUCCESS]'))
      return { type: 'success', cleanText: text.replace('[SUCCESS] ', '') };
    if (text.startsWith('[ERROR]'))
      return { type: 'error', cleanText: text.replace('[ERROR] ', '') };
    ...
    if (text.startsWith('[BUILD]'))
      return { type: 'build', cleanText: text.replace('[BUILD] ', '') };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-border overflow-hidden bg-[#1a1a2e]">
        ...
        {/* Terminal content */}
        <div ref={terminalRef} className="h-[350px] overflow-y-auto p-4 font-mono text-sm">
          {logs.map((log) => (
            <div key={log.id} className={cn('leading-relaxed', getLogColor(log.type))}>
