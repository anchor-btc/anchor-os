  ...
  return (
    <div className="space-y-8">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  {selectedPreset === preset.id && <Check className="w-5 h-5 text-primary" />}
                </div>
                <p className="text-sm text-muted-foreground">{preset.description}</p>

                {preset.id !== 'custom' && (
                  <p className="text-xs text-muted-foreground">
                    {t('setup.preset.servicesCount', '{{count}} services', {
                      count: preset.services.length,
                    })}
                  </p>
                )}
      ...
      <div className="flex justify-between pt-4">
        ...
        <Button onClick={() => onSelect(selectedPreset)}>{t('common.continue', 'Continue')}</Button>
      </div>
    </div>
  );
}
