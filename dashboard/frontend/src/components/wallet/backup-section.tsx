...
export function BackupSection({ t }: BackupSectionProps) {
  ...
  const { data: mnemonicData, isLoading: mnemonicLoading } = useQuery({
      ...
      <Section>
        ...
        {walletInfo ? (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
            <InfoItem label="Fingerprint" value={walletInfo.fingerprint} mono />
            <InfoItem label="Network" value={walletInfo.network} />
            <InfoItem label="Address Type" value={walletInfo.address_type} />
            <InfoItem label="Derivation Path" value={walletInfo.derivation_path} mono />
            <InfoItem label="Addresses Used" value={walletInfo.addresses_used.toString()} />
            <InfoItem
              label="BDK Enabled"
        ...
        <Section>
          ...
          {showMnemonic ? (
                  ...
                  <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2">
                    {mnemonicData.words.map((word, index) => (
                      ...
                      >
                        ...
                        {copiedWord === index && <Check className="w-3 h-3 text-success ml-auto" />}
                      </button>
                    ))}
                  </div>
            ...
            <p className="text-muted-foreground text-sm">
              Click &quot;Reveal&quot; to show your 24-word recovery phrase. Make sure no one is
              watching.
            </p>
          )}
        </Section>
      ...
      {showExportModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm">
          <div className="bg-card border border-border rounded-xl p-6 w-full max-w-md shadow-xl">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Enter a strong password to encrypt your backup. You will need this password to restore
              your wallet.
            </p>
            ...
            {exportMutation.isError && (
              <p className="text-sm text-error mt-4">Failed to export backup. Please try again.</p>
            )}
          </div>
        </div>
      )}
            ...
            {uploadedBackup && (
              <div className="mb-4 p-3 bg-muted rounded-lg text-sm">
                <p>
                  ...
                  {uploadedBackup.network || 'Unknown'}
                </p>
                <p>
                  <span className="text-muted-foreground">Created:</span>{' '}
                  {uploadedBackup.created_at
                    ? new Date(uploadedBackup.created_at).toLocaleString()
                    : 'Unknown'}
                </p>
                <p>
                  <span className="text-muted-foreground">Locked UTXOs:</span>{' '}
                  {uploadedBackup.locked_utxos?.length ?? 0}
                </p>
              </div>
            )}
...
function InfoItem({
  ...
  return (
    <div>
      ...
      <p className={cn('text-sm truncate', mono && 'font-mono', color || 'text-foreground')}>
        {value}
      </p>
    </div>
  );
