...
import { Section, SectionHeader, Grid, StatCard } from '@/components/ds';
...
export function NodeOverviewTab({ status, nodeRunning, refetch }: NodeOverviewTabProps) {
  ...
  if (!nodeRunning) {
    return (
      <Section className="text-center py-12">
        ...
        <h2 className="text-xl font-semibold text-foreground mb-2">{t('node.notRunning')}</h2>
        <p className="text-muted-foreground mb-4">
          {t('node.startNodeCmd')}:{' '}
          <code className="bg-muted px-2 py-1 rounded">docker compose up -d core-bitcoin</code>
        </p>
      </Section>
    );
  }
      ...
      {blockchain && mempool && network && (
        <>
          <Grid cols={{ default: 1, lg: 2 }} gap="lg">
            {/* Blockchain Info */}
            <Section>
              <SectionHeader icon={Database} iconColor="blue" title={t('node.blockchainInfo')} />
              <div className="space-y-3">
                ...
                <InfoRow
                  label={t('node.pruned')}
                  value={blockchain.pruned ? t('common.yes') : t('common.no')}
                />
              </div>
            </Section>
            ...
            {/* Mempool Info */}
            <Section>
              <SectionHeader icon={Zap} iconColor="yellow" title={t('node.mempoolInfo')} />
              <div className="space-y-3">
                <InfoRow
                  label={t('node.loaded')}
                  value={mempool.loaded ? t('common.yes') : t('common.no')}
                />
                <InfoRow label={t('wallet.transactions')} value={mempool.size.toLocaleString()} />
                <InfoRow label={t('node.size')} value={`${(mempool.bytes / 1024).toFixed(2)} KB`} />
                <InfoRow
          ...
          {/* Network Info */}
          <Section>
            <SectionHeader icon={Network} iconColor="cyan" title={t('node.networkInfo')} />
            <Grid cols={{ default: 1, md: 2, lg: 3 }} gap="md">
              <InfoRow label={t('node.version')} value={network.version.toString()} />
              <InfoRow label={t('node.subversion')} value={network.subversion.replace(/\//g, '')} />
              <InfoRow
                label={t('node.protocolVersion')}
                value={network.protocolversion.toString()}
              />
              <InfoRow label={t('node.connections')} value={network.connections.toString()} />
              <InfoRow label={t('node.inbound')} value={network.connections_in.toString()} />
              <InfoRow label={t('node.outbound')} value={network.connections_out.toString()} />
              <InfoRow
