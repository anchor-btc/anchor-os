...
export function StatusDot({ status, label, size = 'md', className }: StatusDotProps) {
  ...
  return (
    <div className={cn('inline-flex items-center gap-2', className)}>
      <div className={cn('rounded-full shrink-0', sizeClasses[size], statusClasses[status])} />
      {showLabel && <span className={cn('text-sm', statusTextClasses[status])}>{labelText}</span>}
    </div>
  );
}
