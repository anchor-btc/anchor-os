...
export function checkIncompatibility(serviceId: string, selectedServices: string[]): string | null {
...
export function getAllDependencies(serviceId: string, visited: Set<string> = new Set()): string[] {
...
export function validateSelection(selectedServices: string[]): {
  ...
  for (const service of selectedServices) {
    ...
    if (incompatible) {
      errors.push(`Cannot select both "${service}" and "${incompatible}" - they use the same port`);
    }
  }
