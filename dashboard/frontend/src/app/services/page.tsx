...
import { fetchContainers, startContainer, stopContainer, fetchContainerLogs } from '@/lib/api';
import { ServiceCard } from '@/components/service-card';
import { Loader2, Play, Square, Terminal, X, Filter, Server } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredContainers.map((container) => (
            <div key={container.id} className="relative">
              <ServiceCard container={container} onAction={() => refetch()} />
              <button
          ...
          <div className="w-full max-w-4xl max-h-[80vh] bg-card border border-border rounded-xl overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-border">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-medium text-foreground">{selectedContainer} logs</h3>
              </div>
              <button
