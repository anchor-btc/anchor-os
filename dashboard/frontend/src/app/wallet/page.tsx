...
import { PageHeader, Section, SectionHeader, Tabs, Tab, ActionButton } from '@/components/ds';
...
export default function WalletPage() {
  ...
  const { data: lockedUtxos } = useQuery({
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'assets' && <LockedAssetsSection t={t} />}

      {activeTab === 'receive' && (
        <Section>
          <SectionHeader icon={QrCode} iconColor="primary" title={t('wallet.receiveBitcoin')} />
          ...
          {!addressMutation.data ? (
            ...
            <div className="flex flex-col items-center gap-6">
              <div className="p-4 bg-white rounded-xl">
                <QRCodeSVG value={`bitcoin:${addressMutation.data.address}`} size={200} level="H" />
              </div>
      ...
      {activeTab === 'backup' && <BackupSection t={t} />}
    </div>
  );
}
  ...
  return (
    <div className="flex items-center justify-between p-4 hover:bg-muted/50 transition-colors">
      <div className="flex items-center gap-3">
        ...
        >
          {isReceive ? <ArrowDownLeft className="w-5 h-5" /> : <ArrowUpRight className="w-5 h-5" />}
        </div>
        <div>
          <div className="flex items-center gap-2">
      ...
      <div className="text-right">
        <p className={cn('font-medium font-tabular', isReceive ? 'text-success' : 'text-error')}>
          {isReceive ? '+' : '-'}
