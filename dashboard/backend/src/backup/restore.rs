...
use crate::backup::database::{get_anchor_databases, DatabaseConfig};
...
fn get_db_container_map() -> HashMap<&'static str, Vec<&'static str>> {
    ...
    map.insert(
        "anchor",
        vec!["core-indexer", "core-wallet", "anchor-dashboard-backend"],
    );
...
async fn restore_database(dump_file: &str, config: &DatabaseConfig) -> Result<()> {
    ...
    if config.port == 5432 {
        ...
        let _ = Command::new("sh").arg("-c").arg(&drop_cmd).output().await;
        ...
        if !output.status.success() {
            ...
            return Err(anyhow::anyhow!(
                "Failed to restore MySQL database: {}",
                stderr
            ));
        }
...
async fn restore_volume_from_tar(tar_file: &str, volume_name: &str) -> Result<()> {
    ...
    let output = Command::new("docker")
        .args([
            ...
            "-v",
            &format!("{}:/backup:ro", tar_dir),
            "alpine",
            "sh",
            "-c",
            &format!(
                "rm -rf /data/* && tar -xf /backup/{} -C /data",
                tar_filename
            ),
        ])
        .output()
...
pub async fn full_restore(
    ...
    if let Err(e) = extract_snapshot(&engine, target, snapshot_id, &restore_path).await {
        result
            .errors
            .push(format!("Failed to extract snapshot: {}", e));
    ...
    info!(
        "Found {} database dumps and {} volume tars",
        db_dumps.len(),
        vol_tars.len()
    );
    ...
    if let Err(e) = stop_containers(&all_containers).await {
        result
            .errors
            .push(format!("Warning: Error stopping containers: {}", e));
    }
    ...
    for (db_name, dump_path) in &db_dumps {
        ...
        if let Some(config) = db_config {
            ...
            result
                .errors
                .push(format!("No config for database: {}", db_name));
        }
    }
    ...
    if let Err(e) = start_containers(&all_containers).await {
        result
            .errors
            .push(format!("Warning: Error starting containers: {}", e));
    }
