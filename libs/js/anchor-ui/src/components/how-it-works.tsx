...
export function HowItWorks({
  ...
  return (
    <div className={className}>
      {title && (
        <h2 className="text-2xl font-bold text-white mb-8 text-center tracking-tight">{title}</h2>
      )}
      <div className={cn('grid gap-6', gridCols)}>
        {steps.map((item, index) => (
          ...
          >
            ...
            >
              <span className={cn('text-2xl font-bold bg-clip-text text-transparent', gradient)}>
                {item.step}
              </span>
            </div>
